...
pub struct DeepInFreeClient {
    client: Arc<Client>,
}

impl DeepInFreeClient {
        ...
        F: FnMut(&str) -> Result<()>,
    {
        let request_body = format!(
            r#"{{"model":"{}","messages":[{{"role":"user","content":"{}"}}],"stream":true,"stream_options":{{"include_usage":true,"continuous_usage_stats":true}},"max_tokens":{}}}"#,
            model, message, max_tokens
        );

        let common_headers = create_common_headers();
        let (final_url, cpi_url) = self.setup_proxy(&common_headers)?;
        self.make_streaming_request(&final_url, &cpi_url, &request_body, callback)
    }

    fn setup_proxy(&self, common_headers: &HeaderMap) -> Result<(String, String)> {
        let res1 = self
            .client
            .get(PROXY_BASE_URL)
        ...
        let server_info = select_proxy_server(&self.client, &html_body2, PROXY_BASE_URL)?;
        ...
        let cpi_url = res_cpi.url().to_string();
        let cpi_page_html = res_cpi.text()?;
        let document_cpi = Html::parse_document(&cpi_page_html);

        let data_r =
            extract_attribute_from_html(&document_cpi, SELECTOR_SCRIPT_INIT_SCRIPT, "data-r")?;
        let final_url = String::from_utf8(Base64Standard.decode(data_r)?)?;

        Ok((final_url, cpi_url))
    }

    fn make_streaming_request<F>(
        &self,
        final_url: &str,
        cpi_url: &str,
        request_body: &str,
        ...
        F: FnMut(&str) -> Result<()>,
    {
        let res_stream = self
            ...
            .header(
                ORIGIN,
                reqwest::Url::parse(cpi_url)?.origin().ascii_serialization(),
            )
            .header(REFERER, cpi_url)
